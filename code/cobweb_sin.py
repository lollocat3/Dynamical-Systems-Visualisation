...
def plot(a, x0=0.1, iterations=50):
	...
	plt.title(r'$\textrm{Cobweb Plot:} \; f(x_n) = \sin(x_n)$', fontsize = 20)
	plt.xlabel(r'$x_n$', fontsize = 20)
	plt.ylabel(r'$x_{n+1}$', fontsize = 20)

	x = np.linspace(0, 3, 1000)
	fx = np.sin(x)	
	plt.plot(x, fx, label=r"$f(x_n) = \sin(x_n)$")
	plt.legend(fontsize = 15)
	
	plt.plot([0, 1.5], [0, 1.5], color="red", label = r'$f(x_n) = x_n$')
	plt.legend(fontsize = 15)

	last_x, last_y = x0, 0
	for _ in range(iterations):
		#next_x = a * last_x * (1 - last_x)
		next_x = np.sin(last_x)
	...
	plt.savefig('/Users/lorenzo/Desktop/Python/sin_cobweb.pdf', bbox_inches = 'tight',
